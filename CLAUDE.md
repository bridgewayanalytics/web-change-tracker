...
2. If `agenda_items == true`: `_resolve_agenda_items()` is status-aware — items with `status == "New"` POST to `wf/create-agenda-item/` (payload: `{title, official_title, reference_id, chronicle_topics: [ids], alert_id, space_id}`; response: `{"bubble_id": "..."}`); items with `status == "Existing"` or `"Updated"` call `_find_agenda_item_by_title()` which searches `/obj/agendaitem` by "BA title" field and returns the existing `_id`. IDs from both paths collected as `agenda_item_ids` for steps 3 and 4. When `action == "agenda_items"`, patches `eidarix_agenda_item_ids` on the JSONL row and returns immediately.
