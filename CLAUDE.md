...
2. Re-runs `extract_page_change()` + `extract_document_data()` with current DynamoDB config. Uses the **original row's `run_timestamp`** as the pipeline run time injected into the agent context (not the current wall-clock time) — ensures the agent sees the same temporal context as the original run.
...
- `bubble/page_change_agent.py` — Page change RAG agent (`extract_page_change()`, `_unwrap_alerts()`, `_sanitize_schema_for_openai()`, `get_config_hash()`). Calls `get_org_tree()` on every invocation to inject the live Bubble org hierarchy into the agent's user message. The `=== TARGET CONTEXT ===` block injected into the user message contains only three fields: `Page`, `URL`, and `Pipeline run time (Eastern)`. The `Org path`, `Group`, and `Tags` fields were removed because they contained internal routing codes (e.g. `NAIC > E > Working Groups`, `working_group`, `committee:E`) that leaked into agent context and caused bad org outputs. The hardcoded alert type list that was previously appended to the user message was also removed — alert type definitions belong in the DynamoDB system prompt only.
- `bubble/document_agent.py` — Document matching RAG agent (`extract_document_data(document_name, document_url, pdf_text=None, text_limit=None, alert_context=None)`, two-step pgvector enforcement). **PDF text is never injected into the agent prompt.** `pdf_text` is fetched solely for upload to pgvector via `ingest_and_arm()` — the agent accesses document content exclusively through knowledge base search tools. `text_limit` is retained for call-site compatibility but is unused. Also used for transcript extraction — `extraction_source: "transcript"` distinguishes those rows in `document_extractions_table.jsonl`. `_fetch_pdf_text(url)` handles semicolon-separated multi-URL `library_item_url` fields by splitting on `;` and calling `_fetch_single_pdf()` for each URL, returning the first successful result. Non-PDF URLs (`.docx`, web pages) are silently skipped. Calls `get_org_tree()` on every invocation and appends the full NAIC org hierarchy under `=== ORGANIZATION REFERENCE ===` in the user message. When `alert_context` (parent alert row dict) is provided, also injects `=== ALERT CONTEXT ===` with `organization`, `alert_type`, `event_title`, and `event_start_date_time` so the agent has the same context the page_change_agent decided. All call sites in `spike.py` pass `alert_context=agent_output` (normal pipeline, rerun, both transcript paths).
...
- `bubble/bubble_sync_classifier.py` — `classify_alert(alert) → BubbleSyncPlan` pure classifier; stamps `bubble_action` on applicable alerts. `enrich_with_doc_extraction(bubble_action, extraction)` merges doc extraction fields (description, date, type, chronicle topics) into previews. No I/O. `_build_agenda_previews()` reads flat-schema `agenda_item_title_chronicle_topics` first, falls back to old nested-schema `agenda_item_title_and_chronicle_topics` for backward compatibility. Includes `status` field on each agenda item preview: `"New"` = create a new Bubble agendaitem object, `"Existing"`/`"Updated"` = look up by title and link. `_parse_doc_extraction_agenda_items()` parses the doc extraction `agenda_items` string — handles both `; ` and `|` as entry separators. See `bubble_action` field docs below.
- `bubble/bubble_sync.py` — `sync_alert(agent_call_id, action="all")` real Bubble sync executor. `action` parameter controls which step runs: `"all"` = full sequence, `"agenda_items"` = create/link agenda items only (patches `eidarix_agenda_item_ids` onto row then returns), `"library_item"` = create/update library item only (picks up `eidarix_agenda_item_ids` from already-patched row), `"event"` = create/update event only. Processing order: (1) `_resolve_agenda_items()` — reads from `bubble_action.agenda_item_previews` as primary source (falls back to flat alert fields for old rows without previews); strips "New - "/"Existing - "/"Updated - " status prefixes from titles via `_strip_status_prefix()` before all Bubble API calls; status-aware: `"New"` items POST to `wf/create-agenda-item/`; `"Existing"`/`"Updated"` items call `_find_agenda_item_by_title()` (searches Bubble `/obj/agendaitem` by "BA title" field); failures raise `RuntimeError` (no longer silently swallowed — the whole sync errors if any agenda item fails). (2) library item via `wf/create-library-item/`. (3) event via `wf/create-event` or `wf/update-event`; UPDATE payload contains only linking fields (`id, alert_id, space_id, chronicle_topics, agenda_items, agenda`) — NOT datetime/type/orgs which are already on the record. All writes go through Mori's Eidarix workflow endpoints, NOT the Bubble Data API directly. Resolves org names → IDs and chronicle topic names → IDs; resolves library item type name → ID via `libraryitemtype` endpoint. Stamps `eidarix_agenda_item_ids`, `bubble_library_item_id`, `bubble_event_id` on the alert row. Triggered via synchronous Lambda invocation from the dashboard route (`InvocationType: RequestResponse`). `EIDARIX_VERSION` env var controls test vs live (default: `"test"`). Full payload spec: `docs/bubble_sync_payload_spec.json`. **Error handling:** org resolution runs inside a try/except — missing orgs log a warning and continue with empty org list (do not raise). Chronicle topic resolution similarly warns on misses. UPDATE lookup raises `RuntimeError` on not-found (previously silently skipped). When an event has no chronicle topics, falls back to empty topic list rather than erroring.
...
| `scripts/set_doc_qa_namespaces.py` | One-time utility to set `pgvector_namespaces` for `chat:document-extraction-qa-agent` in DynamoDB. Use `--dry-run` to inspect current value without writing. |
...
| File | Purpose |
|------|---------|
| `eval/run_eval.py` | Entry point — calls `_load_secrets()` to load OpenAI + DB creds from SSM, then orchestrates full eval run. `main()` wraps the `run()` call in a top-level try/except that logs the full traceback before re-raising, so eval failures are visible in CloudWatch. |
| `eval/row_selector.py` | Loads eligible rows by querying Bubble for all calendar items and library items, then matching alert rows against them via `bubble_action.match_search`. A row qualifies when its corresponding Bubble record actually exists — meaning a human has made editorial decisions (chronicle topics, newsreel relevance) that provide eval ground truth. Rows specified via `--agent-call-ids` bypass the Bubble check entirely. Applies limit at the agent-call level but returns ALL rows per selected call (siblings included). |
| `eval/html_fetcher.py` | Fetches before/after HTML snapshots from S3 for each row |
| `eval/context_builder.py` | Previously built multi-section pgvector + Bubble context injected into the eval agent. **No longer called** — eval agent now runs on HTML snapshots only (`reference_context = ""`). File retained but not used by the pipeline. |
| `eval/eval_agent.py` | Calls `chat:web-extraction-qa-agent` (DynamoDB config) — one call per row, returns per-field scores. Accepts optional `sibling_rows` list; when present, injects a sibling summary block so the agent knows which other documents came from the same run and doesn't penalize a row for content that belongs to a sibling. Injects the live org tree via `get_org_tree()` (same call as `page_change_agent.py`) so the agent can verify organization field values against the full NAIC org hierarchy. The stale "Bubble ground truth" reference was removed from the instructions — `reference_context` is always an empty string (context_builder.py is no longer called). |
...
**Agent config:** `chat:web-extraction-qa-agent` in DynamoDB `chatkit_production_config`. System instructions and rubric configured in Bubble admin, same pattern as `web-tracking-agent`.
...
## Document Extraction QA Pipeline

Parallel QA pipeline for `document_extractions_table.jsonl` rows. Evaluates extraction output by accessing the source document exclusively via pgvector search tools — PDF text is never injected into the prompt. The document is vectorized via `ingest_and_arm()` before the agent runs; if already cached from the extraction pipeline run, no re-upload occurs.

**Entry point:** `python3 -m eval.run_doc_eval`

| Flag | Description |
|------|-------------|
| `--limit N` | Evaluate last N eligible rows (default 20) |
| `--agent-call-ids a,b` | Evaluate specific rows by agent_call_id |
| `--dry-run` | Print selected rows without calling agent |

Eligible rows: excludes `extraction_source == "transcript"` rows and rows with no real `library_item_url`.

**Pipeline modules (`eval/`):**

| File | Purpose |
|------|---------|
| `eval/run_doc_eval.py` | Entry point — deduplicates by `(library_item_url, eval_row_key)` keeping most recent row, groups rows by `(doc_extraction_id, document_url_web_tracking_agent)`, evaluates each group in a **single agent call** (all agenda-item rows together — no batch splitting). Each group runs in a fresh subprocess. `_is_placeholder_scores()` guard handles the edge case where the agent doesn't reach all rows in a group — those rows receive empty scores rather than false results. |
| `eval/doc_eval_agent.py` | QA agent — fetches PDF text from `library_item_url` via `_fetch_pdf_text()` for vectorization only (via `ingest_and_arm()`); **PDF text is never injected into the prompt**. If pdf_text fetch fails, falls back to `arm_if_ready()` to reuse a cached namespace. Prompt contains only document title, URL, and an instruction to use knowledge base search tools. `_fetch_pdf_text()` handles semicolon-separated multi-URL fields. Uses `chat:document-extraction-qa-agent` DynamoDB config; pgvector-enabled when env creds present. `_EXCLUDE_KEYS` controls which fields are omitted from the extraction JSON sent to the QA agent — excludes pipeline metadata (`run_id`, `run_timestamp`, `target_id`, `source_url`, `agent_call_id`, `config_hash`, `last_rerun_at`), library item identity fields (`library_item_title`, `library_item_url`, `library_item_file_name`), and eval bookkeeping (`eval_run_id`, `eval_timestamp`, `eval_scores`, `eval_row_key`, `extraction_source`). All schema output fields including `organization_publisher` are sent to the QA agent and should be scored. `_ALERT_INCLUDE_KEYS = {"organization", "alert_type", "event_title", "event_start_date_time"}` — exactly matches what `document_agent.py` injects via `alert_context` so both agents operate on the same parent-alert context. |
| `eval/doc_result_store.py` | Upserts results into `alerts/doc_eval_results_table.jsonl` keyed by `eval_row_key`. Before upserting, removes ALL existing entries for the same `(agent_call_id, library_item_url)` pair regardless of key format — prevents stale entries under old keys from persisting alongside new ones. Also stamps `eval_row_key` back onto source rows in `document_extractions_table.jsonl` so the dashboard can do a direct field lookup instead of recomputing the key. |
| `eval/doc_score_reporter.py` | Reads all of `doc_eval_results_table.jsonl`, computes overall and per-field accuracy (correct / total, partials tracked separately), writes `alerts/doc_eval_score_report.json` to S3. Called automatically at the end of every `run_doc_eval.run()`. Non-fatal: a failure logs a warning and does not propagate. Fields sorted ascending by accuracy (weakest first). |

**Agent config:** `chat:document-extraction-qa-agent` in DynamoDB `chatkit_production_config`.

**Storage:** `alerts/doc_eval_results_table.jsonl` — same upsert pattern as `eval_results_table.jsonl`.

**ECS trigger:** dashboard fires `POST /api/eval/documents { agent_call_id }` → ECS RunTask CMD override `["python", "-m", "eval.run_doc_eval", "--agent-call-ids", "<id>"]`.
...
```json
{
  "bubble_action": {
    ...
    "agenda_item_previews": [{ "title", "status", "chronicle_topics", "official_title", "reference_id" }],
    "notes": "<alert_type>"
  }
}
```
...
Dashboard shows "Sync to Bubble" button on rows with `bubble_action` set and no `bubble_sync_status`. Clicking opens a preview modal. After confirm, `/api/bubble/sync` invokes the `web-change-tracker-prod-bubble-sync` Lambda **synchronously** and returns the result directly — no polling.
...
2. If `agenda_items == true`: `_resolve_agenda_items()` reads from `bubble_action.agenda_item_previews` (primary source — reflects any user edits/deletions from the modal); falls back to flat `agenda_item_title_chronicle_topics` field for old rows without previews. `_strip_status_prefix()` removes "New - "/"Existing - "/"Updated - " prefixes from all titles and reference_ids before any Bubble API call. Status-aware: `"New"` items POST to `wf/create-agenda-item/`; `"Existing"`/`"Updated"` items search `/obj/agendaitem` by title. Failures raise `RuntimeError` — the whole sync stops and the row is marked `bubble_sync_status: "error"`. IDs collected as `agenda_item_ids` for steps 3 and 4. When `action == "agenda_items"`, patches `eidarix_agenda_item_ids` on the JSONL row and returns immediately.
...
5. If `event == "create"`: POST to `wf/create-event`. Payload: `{chronicle_topics: [ids], alert_id, space_id, agenda_items: [ids_from_step2], agenda: [library_item_id_from_step3], start_datetime, end_datetime, location_url, phone_and_access_code, type: type_id, organizations: [ids]}`. `type` ID resolved at runtime from `/obj/calendaritemtype`. `agenda` = list of library item Bubble IDs. `agenda_items` = list of agenda item Bubble IDs.
6. If `event == "update"`: **trimmed payload only** — `{id, alert_id, space_id, chronicle_topics: [ids], agenda_items?, agenda?}`. Does NOT re-send datetime/type/orgs (already on the record; sending extra fields causes Eidarix "length isn't valid" errors).
...
- Data API base URL (reads/lookups): `https://eidarix.bridgewayanalytics.com/api/1.1/obj/` (no version — use space constraint to target correct space)
- Auth: `Authorization: Bearer <BUBBLE_API_KEY>` (from `bubble/bridgemind.py`)
...
- **Dynamic org tree:** `bubble/org_tree.py` fetches the live Bubble org hierarchy at runtime via `get_org_tree()` (30-min in-process cache, DFS traversal sorted by `Order`, `"-" * level + Name` format). Falls back to `prompts/org_tree.txt` if Bubble is unavailable. Called from `page_change_agent.py` and `document_agent.py` for every agent invocation — org structure changes in Bubble propagate automatically within 30 minutes.
- **ECS entrypoint.sh:** Supports running arbitrary Python commands via CMD override (e.g., `python scripts/backfill_document_extractions.py`). If `$1` is `python`, the full command is exec'd directly. Default (no args) runs `python spike.py`. The S3 targets download block (when `TARGETS_SOURCE` is an `s3://` URI) is non-fatal — errors in the Python download script are caught internally and print a warning to stderr, and the outer shell command is guarded with `|| echo "WARNING..."` so eval/backfill ECS tasks don't crash when `TARGETS_SOURCE` is not set or the download fails.
...
- **Reranker concurrency:** `rerank_chunks()` in `bubble/pgvector/reranker.py` processes candidates in batches of `_RERANK_CONCURRENCY = 4` concurrent httpx connections. Unbounded `asyncio.gather()` over all candidates simultaneously causes glibc heap corruption (double free) → SIGABRT. Do NOT remove the batching.
