...
- `bubble/document_agent.py` — Document matching RAG agent (`extract_document_data(document_name, document_url, pdf_text=None, text_limit=None, alert_context=None)`, two-step pgvector enforcement). Also used for transcript extraction with `text_limit=40_000` — `extraction_source: "transcript"` distinguishes those rows in `document_extractions_table.jsonl`. `_fetch_pdf_text(url)` handles semicolon-separated multi-URL `library_item_url` fields by splitting on `;` and calling `_fetch_single_pdf()` for each URL, concatenating text results. Non-PDF URLs (`.docx`, web pages) are silently skipped. Calls `get_org_tree()` on every invocation and appends the full NAIC org hierarchy under `=== ORGANIZATION REFERENCE ===` in the user message. When `alert_context` (parent alert row dict) is provided, also injects `=== ALERT CONTEXT ===` with `organization`, `alert_type`, `event_title`, and `event_start_date_time` so the agent has the same context the page_change_agent decided. All call sites in `spike.py` pass `alert_context=agent_output` (normal pipeline, rerun, both transcript paths).
...
Parallel QA pipeline for `document_extractions_table.jsonl` rows. Evaluates extraction output against the actual source PDF content — the QA agent receives the same input the extraction agent used: PDF text from `library_item_url` plus pgvector access.
