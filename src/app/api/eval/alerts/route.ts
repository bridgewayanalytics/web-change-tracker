...
const EVAL_KEY = "alerts/eval_results_table.jsonl";
...
async function loadEvalMap(): Promise<Record<string, EvalEntry>> {
  try {
    ...
    for (const line of body.split("\n")) {
      ...
      if (!trimmed) continue;
      try {
        const row = JSON.parse(trimmed) as EvalEntry;
...
async function saveEvalMap(map: Record<string, EvalEntry>): Promise<void> {
  const body = Object.values(map)
...
// GET — return map of agent_call_id → eval result (scores + summary + timestamp)
export async function GET() {
  try {
    const map = await loadEvalMap();
    // Return a trimmed map — exclude full alert row data to keep response small
    const trimmed: Record<string, Omit<EvalEntry, never>> = {};
    ...
    console.error("[/api/eval/alerts GET]", msg);
    return NextResponse.json({ results: {}, count: 0, error: msg }, { status: 500 });
  }
}

// POST { agent_call_id? } — trigger ECS eval task.
// With agent_call_id: evaluate that specific row only.
// Without agent_call_id: run on all eligible rows (time-window logic in row_selector.py).
// This second form is called on publish to trigger automatic QA.
export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as { agent_call_id?: string } | null;
    const agent_call_id = body?.agent_call_id ?? null;

    const command = agent_call_id
      ? ["python", "-m", "eval.run_eval", "--agent-call-ids", agent_call_id]
    ...
    console.info("[/api/eval/alerts POST] ECS task started taskId=%s agent_call_id=%s", taskId, agent_call_id ?? "all");
    return NextResponse.json({ taskId, taskArn: task.taskArn, mode: agent_call_id ? "single" : "all" });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[/api/eval/alerts POST]", msg);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[/api/eval/alerts DELETE]", msg);
    return NextResponse.json({ error: msg }, { status: 500 });
  }
}
