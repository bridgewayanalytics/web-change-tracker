...
def enrich_refs(
    ...
    *,
    ...
    # --- Agenda items + Chronicle topic via document agent ---
    if use_ai:
        from bubble import document_agent
        for i, r in enumerate(updated_resources):
            name = (r.get("Name") or "").strip()
            url = (r.get("URL") or "").strip()
            pdf_signals = r.get("__pdf_agenda_signals")
            pdf_text = None
            if isinstance(pdf_signals, dict):
                numbered = pdf_signals.get("numbered_items") or []
                if numbered:
                    pdf_text = "\n".join(str(x) for x in numbered[:20])

            result = document_agent.extract_document_data(name, url, pdf_text=pdf_text)

            if result.get("topic_ids"):
                r["topic suggestion"] = result["topic_ids"][0]
                log.info("chatkit: resource[%d] topic -> %s", i, result["topic_ids"][0])
            if result.get("agenda_item_ids"):
                r["__suggested_agenda_items"] = result["agenda_item_ids"]
                log.info("chatkit: resource[%d] agenda items -> %s", i, result["agenda_item_ids"])
