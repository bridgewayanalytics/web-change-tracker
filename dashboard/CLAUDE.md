...
2. **S3** (`alerts/doc_eval_results_table.jsonl`) → read by `/api/eval/documents`; map keyed by `eval_row_key || agent_call_id`. `eval_row_key` format: `doc_extraction_id|url_filename|discriminator`. In `DocExtractionsTable`, `getEvalForRow(row, evalResults)` resolves the key using multiple fallbacks: direct `eval_row_key`, composite keys with std_id/title/official_title/number, prefix scan for `callId|*`, then bare `callId`.
...
| `/api/eval/documents` | GET | Returns `{ results: Record<eval_row_key, DocEvalResult> }` from `alerts/doc_eval_results_table.jsonl`. Key is `eval_row_key` when present on the row, otherwise falls back to `agent_call_id`. |
...
| `/api/eval/documents/report` | GET | Returns `{ report: DocScoreReport | null }` from `alerts/doc_eval_score_report.json`. No cache. Written by `eval/doc_score_reporter.py` after each doc eval run. |
...
- **`page.tsx`** — document extractions page container. Filter bar includes: Page dropdown, **Organization** dropdown (client-side, derived from all API rows), **Document Type** dropdown (client-side), date range, search. QA toggle buttons: **"Has QA Score"** (violet) and **"Imperfect QA Only"** (amber) — mutually exclusive, passed as props to `DocExtractionsTable`. Clear button shows active filter count. Org and doc type filtering applied client-side after API fetch so dropdown options always reflect the full result set. **Score report banner** (`ScoreBanner`) appears between the filter bar and the table when `alerts/doc_eval_score_report.json` is present in S3 (fetched from `/api/eval/documents/report` on mount). Always shows: overall accuracy %, correct/total score counts, row count, field count, last-updated relative time. **By-field table** collapsed by default, expanded on click — shows all fields sorted ascending (weakest first) with a progress bar, accuracy %, and C/~P/I counts. Color thresholds: green ≥ 90%, amber ≥ 70%, red < 70%.
